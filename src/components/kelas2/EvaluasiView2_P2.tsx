...
import { 
  EVALUASI_PG_KELAS2_P2, 
  EVALUASI_PGK_KELAS2_P2, 
  EVALUASI_MENJODOHKAN_KELAS2_P2, 
  EVALUASI_BENAR_SALAH_KELAS2_P2, 
  EVALUASI_URAIAN_KELAS2_P2 
} from '../../data/kelas2Pembelajaran2Data';
...
interface EvaluasiView2_P2Props {
  grade: number;
  lesson: number;
}

export const EvaluasiView2_P2: React.FC<EvaluasiView2_P2Props> = ({ grade, lesson }) => {
  ...
  const handleCalculateScore = () => {
    ...
    EVALUASI_PG_KELAS2_P2.forEach(q => {
    ...
    EVALUASI_PGK_KELAS2_P2.forEach(q => {
    ...
    EVALUASI_MENJODOHKAN_KELAS2_P2.forEach(q => {
    ...
    EVALUASI_BENAR_SALAH_KELAS2_P2.forEach(q => {
    ...
    EVALUASI_URAIAN_KELAS2_P2.forEach(q => {
  ...
  const handlePrintEvaluasi = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Evaluasi Pembelajaran PAI SD Kelas ${grade} Bab ${lesson}: Asmaul Husna</title>
          <style>
        ...
        <body>
          ...
          ${EVALUASI_PG_KELAS2_P2.map((q, idx) => `
            <div class="q-box">
          ...
          ${EVALUASI_PGK_KELAS2_P2.map((q, idx) => `
            <div class="q-box">
              <div class="q-text">${idx + 1}. ${q.question}</div>
              <div class="options-grid">
                ${q.options.map((opt, oIdx) => `
          ...
          ${EVALUASI_MENJODOHKAN_KELAS2_P2.map((q, idx) => `
            <div class="q-box">
              <div class="q-text">${idx + 1}. Pasangkan Asmaul Husna <b>"${q.premise}"</b> dengan maknanya yang tepat:</div>
          ...
          ${EVALUASI_BENAR_SALAH_KELAS2_P2.map((q, idx) => `
            <div class="q-box">
          ...
          <!-- E. URAIAN -->
          <div class="section-title">E. Soal Uraian HOTS (5 Soal)</div>
          ${EVALUASI_URAIAN_KELAS2_P2.map((q, idx) => `
            <div class="q-box">
              <div class="q-text">${idx + 1}. ${q.question}</div>
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800 leading-tight">
              Evaluasi Pembelajaran: Asmaul Husna
            </h2>
            <p className="text-xs text-slate-500">
              Kelas {grade} • Pembelajaran {lesson} (HOTS & Computational Thinking)
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-2 mb-6 pb-3 border-b border-slate-100">
          <span className="w-7 h-7 rounded-lg bg-amber-500 text-white font-black text-xs flex items-center justify-center">A</span>
            ...
            <div key={q.id} className="p-4 rounded-2xl bg-slate-50/70 border border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {q.options.map((opt, oIdx) => {
                  ...
                  let style = 'bg-white border-slate-200 text-slate-700 hover:bg-amber-50';
                  if (isSelected) {
                    style = 'bg-amber-500 text-white border-amber-500 font-bold';
                  }
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
        ...
        <div className="space-y-6">
          {EVALUASI_MENJODOHKAN_KELAS2_P2.map((q, idx) => (
            <div key={q.id} className="p-4 rounded-2xl bg-slate-50/70 border border-slate-100">
              <p className="text-xs sm:text-sm font-bold text-slate-800 mb-3">
                {idx + 1}. Pilihlah pasangan arti yang tepat untuk Asmaul Husna: <span className="text-purple-700 font-black">{q.premise}</span>
              </p>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-2 mb-6 pb-3 border-b border-slate-100">
          ...
          <h3 className="text-base font-bold text-slate-800">Soal Uraian HOTS (5 Soal)</h3>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center relative border border-slate-100 shadow-2xl">
            ...
            <p className="text-xs text-slate-500 mb-4">
              SD Kelas {grade} • Bab {lesson} (Asmaul Husna)
            </p>

            <div className="bg-indigo-50/80 rounded-2xl p-6 border border-indigo-100 mb-6">
              <span className="text-xs font-bold text-indigo-800 block uppercase tracking-wider mb-1">
                Estimasi Skor Nilai:
              </span>
            ...
            <button
              onClick={() => setShowScoreModal(false)}
              className="w-full py-2.5 rounded-xl bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 transition-all shadow-md"
            >
              Tutup Ringkasan
            </button>
          </div>
