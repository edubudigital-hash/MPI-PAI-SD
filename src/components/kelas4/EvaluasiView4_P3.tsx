...
import { 
  ...
  EVALUASI_P3_BS, 
  EVALUASI_P3_ESSAY 
} from '../../data/kelas4Pembelajaran3Data';
...
interface EvaluasiView4P3Props {
  grade: number;
  lesson: number;
}

export const EvaluasiView4_P3: React.FC<EvaluasiView4P3Props> = ({ grade, lesson }) => {
  const dataPG = EVALUASI_P3_PG;
  const dataPGK = EVALUASI_P3_PGK;
  const dataMatching = EVALUASI_P3_MATCHING;
  const dataBS = EVALUASI_P3_BS;
  const dataEssay = EVALUASI_P3_ESSAY;
  ...
  const handleDownloadEvaluasi = () => {
    ...
    printOrDownloadDocument(
      `Evaluasi_PAI_Kelas_${grade}_P${lesson}`,
      `Evaluasi Akhlak Keluarga Kelas ${grade}`,
      bodyHtml
    );
  };
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-md w-full border-2 border-amber-300 shadow-2xl text-center space-y-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">
                Kelas {grade} Pembelajaran {lesson} (Akhlak Keluarga BAB 3)
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-slate-800 dark:to-slate-800/80 border border-amber-200 dark:border-slate-700">
              ...
              <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 pt-2">
                {calculatedScore >= 80
                  ? '🌟 Sangat Baik! Akhlak Mulia dalam Keluarga Terpahami Sempurna!'
                  : calculatedScore >= 60
                  ? '👍 Baik! Teruskan Membiasakan Akhlak Terpuji.'
                  : '💪 Perlu Lebih Banyak Mengamalkan Akhlak Santun di Rumah.'}
              </p>
            </div>
