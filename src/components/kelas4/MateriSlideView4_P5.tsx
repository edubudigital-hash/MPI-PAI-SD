...
import { KELAS4_P5_SLIDES } from '../../data/kelas4Pembelajaran5Data';
...
interface MateriSlideView4P5Props {
  grade: number;
  lesson: number;
}

export const MateriSlideView4_P5: React.FC<MateriSlideView4P5Props> = ({ grade, lesson }) => {
  const [currentSlideIdx, setCurrentSlideIdx] = useState<number>(0);
  const slides = KELAS4_P5_SLIDES;
  ...
  const handleDownloadMateri = () => {
    ...
    let bodyHtml = `
      <div class="header">
        ...
        <p>Berdasarkan Buku Teks PAI BAB 5 • SD Kelas IV</p>
      </div>
    `;
    ...
    printOrDownloadDocument(
      `Materi_PAI_Kelas4_P5_Kisah_Nabi_Muhammad_Makkah.pdf`,
      `Materi PAI Kelas 4 Pembelajaran 5 - Kisah Nabi Muhammad saw. Periode Makkah`,
      bodyHtml
    );
  };
  ...
  const getSlideIcon = (name: string) => {
    switch (name) {
      case 'AlertTriangle': return <AlertTriangle className="w-6 h-6 text-amber-600" />;
  ...
  return (
    <div id="materi-slide-v4-p5" className="space-y-6 max-w-5xl mx-auto">
      {/* Header Bar */}
      <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-700 text-white p-6 rounded-2xl shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-emerald-100 text-sm mt-1">
            Materi Interaktif Berdasarkan Buku Teks Kurikulum Merdeka BAB 5
          </p>
        </div>
        <button
          id="btn-download-materi-p5"
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200/80 overflow-hidden transition-all duration-300 min-h-[460px] flex flex-col justify-between">
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="space-y-3">
            {currentSlide.points.map((pt, idx) => (
              <div key={idx} className="text-slate-700 text-base md:text-lg leading-relaxed">
                {pt}
              </div>
            ))}
          </div>
                ...
                <thead className="bg-emerald-600 text-white font-semibold">
                  <tr>
                    {currentSlide.tableData.headers.map((h, i) => (
                      <th key={i} className="p-3 border-b border-emerald-700">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 bg-white">
                  {currentSlide.tableData.rows.map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-slate-50/50' : 'bg-white'}>
                      {row.map((cell, j) => (
                        <td key={j} className="p-3 text-slate-700 font-medium">{cell}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-slate-50 p-4 border-t border-slate-200/80 flex items-center justify-between">
          <button
            id="btn-prev-bottom-p5"
            onClick={handlePrev}
            disabled={currentSlideIdx === 0}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
              currentSlideIdx === 0 
                ? 'opacity-40 cursor-not-allowed text-slate-400' 
                : 'text-slate-700 hover:bg-white hover:shadow-sm border border-slate-300'
            }`}
          >
            ...
            {slides.map((_, idx) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  idx === currentSlideIdx ? 'bg-emerald-600 w-8' : 'bg-slate-300 hover:bg-slate-400'
                }`}
                title={`Slide ${idx + 1}`}
              />
            ))}
