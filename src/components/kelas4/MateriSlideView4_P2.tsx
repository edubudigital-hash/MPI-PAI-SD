...
import { KELAS4_P2_SLIDES } from '../../data/kelas4Pembelajaran2Data';
...
interface MateriSlideView4P2Props {
  grade: number;
  lesson: number;
}

export const MateriSlideView4_P2: React.FC<MateriSlideView4P2Props> = ({ grade, lesson }) => {
  const slidesData = KELAS4_P2_SLIDES;
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Gift': return <Gift className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />;
      case 'Key': return <Key className="w-6 h-6 text-amber-600 dark:text-amber-400" />;
      case 'Smile': return <Smile className="w-6 h-6 text-teal-600 dark:text-teal-400" />;
      case 'Layers': return <Layers className="w-6 h-6 text-blue-600 dark:text-blue-400" />;
      case 'ShieldCheck': return <ShieldCheck className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />;
      case 'Compass': return <Compass className="w-6 h-6 text-purple-600 dark:text-purple-400" />;
  ...
  const handleDownloadMateri = () => {
    ...
    let bodyHtml = `
      <div class="header">
        ...
        <h2>PEMBELAJARAN ${lesson}: Mengenal dan Meneladani Asmaul Husna (Al-Ghaffār, Al-Wahhāb, Al-Fattāḥ, Al-Laṭīf)</h2>
        <p>Berdasarkan Buku Kurikulum Merdeka PAI Kelas 4 SD - BAB 3</p>
      </div>
    `;
    ...
    slidesData.forEach((s) => {
      bodyHtml += `
        <div class="card">
          ...
          <h3 style="margin: 4px 0; color: #047857; font-size: 14pt;">${s.title}</h3>
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 9pt;">
            <thead>
              <tr style="background-color: #d1fae5; color: #065f46;">
                ${s.tableData.headers.map((h) => `<th style="border: 1px solid #6ee7b7; padding: 6px;">${h}</th>`).join('')}
              </tr>
            </thead>
            <tbody>
      ...
      if (s.arabicExamples) {
        bodyHtml += `
          <div style="margin-top: 10px; background-color: #f0fdf4; padding: 10px; border-radius: 6px; border: 1px solid #bbf7d0;">
            ${s.arabicExamples.map((ex) => `
              <div style="margin-bottom: 8px;">
                <div class="arabic" style="font-size: 20pt; color: #047857;">${ex.arabic}</div>
                <div style="font-size: 9.5pt; font-weight: bold; color: #166534;">${ex.latin} — ${ex.meaning}</div>
                ${ex.note ? `<div style="font-size: 8.5pt; color: #047857; margin-top: 2px;">📌 ${ex.note}</div>` : ''}
              </div>
            `).join('')}
          </div>
        `;
      ...
      if (s.highlightBox) {
        bodyHtml += `
          <div style="margin-top: 8px; padding: 10px; background-color: #fef3c7; border-left: 4px solid #f59e0b; font-size: 9.5pt; border-radius: 4px;">
            <strong style="color: #92400e;">${s.highlightBox.title}</strong>: ${s.highlightBox.content}
          </div>
        `;
      }
    ...
    printOrDownloadDocument(`Materi_PAI_Kelas_${grade}_P${lesson}`, `Materi Asmaul Husna Kelas ${grade}`, bodyHtml);
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <span className="text-xs font-bold text-emerald-200 uppercase tracking-wider block">
              Materi Slide Interaktif PAI & BP • BAB 3
            </span>
            <h3 className="text-xl sm:text-2xl font-black pt-0.5">
              Kelas {grade} • Pembelajaran {lesson}
            </h3>
            <p className="text-xs text-emerald-100 font-medium">
              Asmaul Husna: Al-Ghaffār, Al-Wahhāb, Al-Fattāḥ, dan Al-Laṭīf
            </p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-10 border-2 border-emerald-200 dark:border-emerald-800 shadow-xl space-y-6 relative">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          ...
          <span className="hidden sm:inline-block text-xs font-extrabold text-amber-800 bg-amber-100 dark:bg-amber-950 dark:text-amber-300 px-3 py-1 rounded-full border border-amber-300">
            {slide.subtitle}
          </span>
        </div>
        ...
        <div className="space-y-4 min-h-[240px]">
          ...
          {slide.tableData && (
            <div className="overflow-x-auto pt-2">
              <table className="w-full text-xs sm:text-sm border-collapse border border-emerald-200 dark:border-emerald-800 rounded-xl overflow-hidden shadow-xs">
                <thead>
                  <tr className="bg-emerald-100 dark:bg-emerald-950 text-emerald-900 dark:text-emerald-100 font-extrabold">
                    {slide.tableData.headers.map((h, i) => (
                      <th key={i} className="p-3 border border-emerald-200 dark:border-emerald-800 text-center">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {slide.tableData.rows.map((row, rIdx) => (
                    <tr key={rIdx} className="hover:bg-emerald-50/50 dark:hover:bg-slate-800/50">
                      {row.map((cell, cIdx) => (
                        <td key={cIdx} className={`p-3 border border-slate-200 dark:border-slate-800 text-center font-medium ${cIdx === 1 ? 'font-arabic text-lg text-emerald-700 font-bold' : 'text-slate-800 dark:text-slate-200'}`}>
                          {cell}
                        </td>
                      ))}
                    </tr>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3">
              {slide.arabicExamples.map((ex, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 space-y-1">
                  <div className="font-arabic text-3xl text-emerald-800 dark:text-emerald-300 font-bold text-right leading-loose">
                    {ex.arabic}
                  </div>
                  <div className="text-xs font-bold text-emerald-700 dark:text-emerald-400">
                    {ex.latin} — <span className="text-slate-700 dark:text-slate-300 italic">"{ex.meaning}"</span>
                  </div>
                  {ex.note && (
                    <div className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400 pt-1 border-t border-emerald-200 dark:border-emerald-800">
                      📌 {ex.note}
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100 dark:border-slate-800 pt-5">
          ...
          <div className="flex items-center space-x-3 w-full sm:w-auto justify-end">
            ...
            <button
              ...
              className="flex-1 sm:flex-initial px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs flex items-center justify-center space-x-1.5 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-md active:scale-95"
            >
