...
import { 
  ...
  EVALUASI_P1_BS, 
  EVALUASI_P1_ESSAY 
} from '../../data/kelas4Pembelajaran1Data';
...
interface EvaluasiView4Props {
  grade: number;
  lesson: number;
}

export const EvaluasiView4: React.FC<EvaluasiView4Props> = ({ grade, lesson }) => {
  ...
  const handleCalculateScore = () => {
    ...
    // Essay: 5 soal @ 3 poin = 15 poin (given if user wrote an answer)
    dataEssay.forEach((q) => {
  ...
  const handleDownloadEvaluasi = () => {
    ...
    let bodyHtml = `
      <div class="header">
        ...
        <h2>PEMBELAJARAN ${lesson}: Idgham Bi Ghunnah, Bila Ghunnah & Surah An-Nasr</h2>
        <p>Soal Berbasis HOTS & Computational Thinking • Kurikulum Merdeka</p>
      </div>
    ...
    dataPG.forEach((q) => {
      bodyHtml += `
        <div class="card">
          ...
          <div style="margin-top: 6px; padding-top: 4px; border-top: 1px dashed #cbd5e1; font-size: 8.5pt; color: #059669;">
      ...
      bodyHtml += `
        <div class="card">
          <div style="display: flex; justify-content: space-between; font-size: 9.5pt;">
            ...
            <div style="color: #059669; font-weight: bold;">➡ Pasangan: ${q.rightText}</div>
          </div>
        </div>
      `;
    });
    ...
    bodyHtml += `<div class="section-title">BAGIAN D: BENAR / SALAH DENGAN ALASAN (10 Soal)</div>`;
    dataBS.forEach((q) => {
      bodyHtml += `
        <div class="card">
    ...
    dataEssay.forEach((q) => {
      bodyHtml += `
        <div class="card">
          ...
          <div style="margin-top: 6px; font-size: 8.5pt; color: #059669;">
    ...
    printOrDownloadDocument(`Evaluasi_PAI_Kelas_${grade}_P${lesson}`, `Evaluasi Pembelajaran PAI Kelas ${grade}`, bodyHtml);
  };

  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-white rounded-3xl p-6 shadow-xl border-2 border-amber-300/40 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs sm:text-sm flex items-center space-x-2 shadow-md border border-emerald-500 transition active:scale-95 cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border-2 border-blue-200 dark:border-blue-800 shadow-xl space-y-6">
          ...
          {dataPG.map((q) => {
            ...
            return (
              <div key={q.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3">
                ...
                {q.arabic && (
                  <div className="font-arabic text-2xl text-emerald-800 dark:text-emerald-300 font-bold text-right py-1">
                    {q.arabic}
                  </div>
                )}
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl border text-left text-xs transition cursor-pointer ${
                          isSelected
                            ? 'bg-blue-600 text-white border-blue-500 font-extrabold shadow-sm'
                            : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-blue-50/50'
                        }`}
                      >
                        {opt}
                      </button>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                  {q.options.map((opt, optIdx) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-4 h-4 rounded-sm border flex items-center justify-center shrink-0 ${isChecked ? 'bg-white text-indigo-600' : 'border-slate-400'}`}>
          ...
          {dataMatching.map((q) => (
            <div key={q.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
              <div className="space-y-1">
                <span className="text-xs font-black text-teal-600 dark:text-teal-400 block">Soal #{q.id}</span>
                <div className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">
                  {q.leftText} {q.leftArabic && <span className="font-arabic text-lg text-emerald-600 font-bold ml-1">({q.leftArabic})</span>}
                </div>
              </div>
              ...
              <select
                ...
                className="w-full md:w-72 p-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs text-slate-800 dark:text-slate-200 focus:outline-hidden focus:ring-2 focus:ring-teal-500"
              >
      ...
      {/* SECTION 4: BENAR / SALAH DENGAN KOLOM ISIAN ALASAN (10 SOAL) */}
              ...
              <div key={q.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    {q.arabic && (
                      <div className="font-arabic text-xl text-emerald-800 dark:text-emerald-300 font-bold py-0.5">
                        {q.arabic}
                      </div>
                    )}
                  </div>
      ...
      {showScoreModal && calculatedScore !== null && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-md w-full border-2 border-amber-300 shadow-2xl text-center space-y-6">
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-slate-800 dark:to-slate-800/80 border border-amber-200 dark:border-slate-700">
              ...
              <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 pt-2">
                {calculatedScore >= 80
                  ? '🌟 Sangat Baik! Pemahaman Tajwid & Surah An-Nasr Sangat Sempurna!'
                  : calculatedScore >= 60
                  ? '👍 Baik! Teruskan Berlatih & Murojaah.'
                  : '💪 Perlu Lebih Banyak Berlatih Tajwid Bersama Guru.'}
              </p>
            </div>
            ...
            <div className="flex gap-3 justify-center">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs cursor-pointer flex items-center gap-1.5"
              >
