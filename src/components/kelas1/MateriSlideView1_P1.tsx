...
import { SLIDES_KELAS1_P1 } from '../../data/kelas1Pembelajaran1Data';
...
interface MateriSlideView1_P1Props {
  grade?: number;
  lesson?: number;
}

export const MateriSlideView1_P1: React.FC<MateriSlideView1_P1Props> = ({
  grade = 1,
  lesson = 1,
}) => {
  ...
  const currentSlide = SLIDES_KELAS1_P1[currentSlideIndex];

  const handleNext = () => {
    soundFX.playClick();
    if (currentSlideIndex < SLIDES_KELAS1_P1.length - 1) {
  ...
  const filteredSlides = SLIDES_KELAS1_P1.filter(
    (slide) =>
  ...
  const handleDownloadPDF = () => {
    ...
    const content = `
      ...
      <html>
        <head>
          <title>Materi PAI SD Kelas ${grade} - Pembelajaran ${lesson}: Hijaiyah & Makhraj</title>
          <style>
            @page {
              size: A4;
              margin: 15mm;
        ...
        <body>
          <div class="header">
            ...
            <p>Kelas ${grade} • Pembelajaran ${lesson}: Mahir Membaca dan Menulis Huruf Hijaiyah</p>
          </div>

          ${SLIDES_KELAS1_P1.map(
            (slide) => `
            <div class="slide-box">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800">
              Materi Slide: Membaca & Menulis Huruf Hijaiyah
            </h3>
            <p className="text-xs text-slate-500">
              Slide {currentSlideIndex + 1} dari {SLIDES_KELAS1_P1.length}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-emerald-100 shadow-xl overflow-hidden transition-all duration-300">
          ...
          <div className="p-6 md:p-8 bg-gradient-to-r from-emerald-700 via-teal-700 to-emerald-800 text-white flex items-center justify-between border-b border-emerald-600/50">
            <div className="space-y-1">
              <span className="text-xs font-black uppercase px-3 py-1 bg-white/20 text-emerald-100 rounded-full border border-white/30 backdrop-blur-xs">
                Slide {currentSlide.id} / {SLIDES_KELAS1_P1.length} • {currentSlide.badge}
              </span>
          ...
          <div className="p-6 md:p-10 space-y-6">
            ...
            {currentSlide.arabicText && (
              <div className="p-6 bg-emerald-50 rounded-2xl border border-emerald-200 text-center space-y-2">
                <div className="text-2xl md:text-3xl font-bold text-emerald-800 font-serif leading-loose tracking-wider" dir="rtl">
                  {currentSlide.arabicText}
                </div>
              </div>
            )}
          ...
          <div className="p-5 md:p-6 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            ...
            <div className="text-xs font-extrabold text-slate-500 hidden sm:block">
              {currentSlideIndex + 1} / {SLIDES_KELAS1_P1.length}
            </div>

            <button
              onClick={handleNext}
              disabled={currentSlideIndex === SLIDES_KELAS1_P1.length - 1}
