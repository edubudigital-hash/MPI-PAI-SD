...
import { SLIDES_KELAS1_P3 } from '../../data/kelas1Pembelajaran3Data';
...
interface MateriSlideView1_P3Props {
  grade?: number;
  lesson?: number;
}

export const MateriSlideView1_P3: React.FC<MateriSlideView1_P3Props> = ({
  grade = 1,
  lesson = 3,
}) => {
  ...
  const currentSlide = SLIDES_KELAS1_P3[currentSlideIndex];

  const handleNext = () => {
    soundFX.playClick();
    if (currentSlideIndex < SLIDES_KELAS1_P3.length - 1) {
  ...
  const filteredSlides = SLIDES_KELAS1_P3.filter(
    (slide) =>
  ...
  const handleDownloadPDF = () => {
    ...
    const content = `
      ...
      <html>
        <head>
          <title>Materi PAI SD Kelas ${grade} - Pembelajaran ${lesson}: Subḥānallāh dan Māsyā Allāh</title>
          <style>
            @page {
              size: A4;
              margin: 15mm;
        ...
        <body>
          <div class="header">
            ...
            <p>Kelas ${grade} • Pembelajaran ${lesson}: Mengucapkan Subḥānallāh dan Māsyā Allāh</p>
          </div>

          ${SLIDES_KELAS1_P3.map(
            (slide) => `
            <div class="slide-box">
              ...
              ${slide.arabic ? `
                <div class="arabic-callout">
                  <div class="arabic-text">${slide.arabic}</div>
                  <div style="font-size: 12px; font-weight: bold; color: #1e293b; margin-top: 4px;">${slide.transliteration}</div>
                  <div style="font-size: 11px; color: #047857;">Artinya: "${slide.meaning}"</div>
                </div>
              ` : ''}
    ...
    <div className={`space-y-6 ${isFullscreen ? 'fixed inset-0 z-50 bg-slate-900 p-6 overflow-y-auto' : ''}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800">
              Materi Slide: Kalimat Subḥānallāh & Māsyā Allāh
            </h3>
            <p className="text-xs text-slate-500">
              Slide {currentSlideIndex + 1} dari {SLIDES_KELAS1_P3.length}
            </p>
          </div>
        </div>
      ...
      {searchQuery ? (
        ...
        <div className="bg-white rounded-3xl border border-emerald-100 shadow-xl overflow-hidden transition-all duration-300">
          {/* SLIDE HEADER */}
          <div className="p-6 md:p-8 bg-gradient-to-r from-teal-700 via-emerald-700 to-teal-800 text-white flex items-center justify-between border-b border-emerald-600/50">
            <div className="space-y-1">
              <span className="text-xs font-black uppercase px-3 py-1 bg-white/20 text-emerald-100 rounded-full border border-white/30 backdrop-blur-xs">
                Slide {currentSlide.id} / {SLIDES_KELAS1_P3.length} • {currentSlide.badge}
              </span>
          ...
          <div className="p-6 md:p-10 space-y-6">
            ...
            {/* ARABIC CALLOUT */}
            {currentSlide.arabic && (
              <div className="p-6 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl border border-emerald-200 text-center space-y-2 shadow-inner">
                <div className="text-3xl md:text-4xl font-bold text-emerald-800 font-serif leading-loose">
                  {currentSlide.arabic}
                </div>
          ...
          <div className="p-5 md:p-6 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            ...
            <div className="text-xs font-extrabold text-slate-500 hidden sm:block">
              {currentSlideIndex + 1} / {SLIDES_KELAS1_P3.length}
            </div>

            <button
              onClick={handleNext}
              disabled={currentSlideIndex === SLIDES_KELAS1_P3.length - 1}
