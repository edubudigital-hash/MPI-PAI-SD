...
import { 
  EVALUASI_PG_KELAS1_P5, 
  EVALUASI_PGK_KELAS1_P5, 
  EVALUASI_MENJODOHKAN_KELAS1_P5, 
  ALL_MATCH_OPTIONS_P5, 
  EVALUASI_BENAR_SALAH_KELAS1_P5, 
  EVALUASI_URAIAN_KELAS1_P5 
} from '../../data/kelas1Pembelajaran5Data';
...
interface EvaluasiView1_P5Props {
  grade?: number;
  lesson?: number;
}

export const EvaluasiView1_P5: React.FC<EvaluasiView1_P5Props> = ({
  grade = 1,
  lesson = 5,
}) => {
  // USER ANSWERS STATE
  ...
  const handleCalculateScore = () => {
    ...
    EVALUASI_PG_KELAS1_P5.forEach((q) => {
    ...
    EVALUASI_PGK_KELAS1_P5.forEach((q) => {
    ...
    EVALUASI_MENJODOHKAN_KELAS1_P5.forEach((q) => {
    ...
    EVALUASI_BENAR_SALAH_KELAS1_P5.forEach((q) => {
    ...
    EVALUASI_URAIAN_KELAS1_P5.forEach((q) => {
    ...
    const content = `
      ...
      <html>
        ...
        <body>
          ...
          ${EVALUASI_MENJODOHKAN_KELAS1_P5.map(
            (q, idx) => `
            <div class="question-item">
          ...
          ${EVALUASI_BENAR_SALAH_KELAS1_P5.map(
            (q, idx) => `
            <div class="question-item">
              <div class="q-num">${idx + 1}. Pernyataan: "${q.statement}"</div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-bold px-2.5 py-0.5 bg-emerald-100 text-emerald-800 rounded-full">
              Evaluasi Pembelajaran PAI Kelas 1 P5
            </span>
            <h3 className="text-xl font-black text-slate-800 mt-1">
              Lembar Evaluasi Kisah Nabi Adam a.s.
            </h3>
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-md p-6 md:p-8 space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {EVALUASI_PG_KELAS1_P5.map((q, idx) => (
        ...
        <div className="space-y-6 pt-4 border-t">
          ...
          <div className="space-y-6">
            {EVALUASI_PGK_KELAS1_P5.map((q, idx) => (
              <div key={q.id} className="p-5 bg-slate-50/70 rounded-2xl border border-slate-200 space-y-3">
        ...
        <div className="space-y-6 pt-4 border-t">
          <div className="p-3 bg-teal-50 rounded-2xl border border-teal-200">
            ...
            <p className="text-xs text-teal-800">Pilihlah pasangan yang tepat untuk tiap pernyataan!</p>
          </div>

          <div className="space-y-4">
            {EVALUASI_MENJODOHKAN_KELAS1_P5.map((q, idx) => (
              <div key={q.id} className="p-5 bg-slate-50/70 rounded-2xl border border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <div className="w-full md:w-64">
                  ...
                  >
                    ...
                    {ALL_MATCH_OPTIONS_P5.map((opt, oIdx) => (
        ...
        <div className="space-y-6 pt-4 border-t">
          ...
          <div className="space-y-4">
            {EVALUASI_BENAR_SALAH_KELAS1_P5.map((q, idx) => (
              <div key={q.id} className="p-5 bg-slate-50/70 rounded-2xl border border-slate-200 space-y-3">
        ...
        <div className="space-y-6 pt-4 border-t">
          ...
          <div className="space-y-6">
            {EVALUASI_URAIAN_KELAS1_P5.map((q, idx) => (
              <div key={q.id} className="p-5 bg-slate-50/70 rounded-2xl border border-slate-200 space-y-3">
                <div className="font-extrabold text-sm text-slate-800">{idx + 1}. {q.question}</div>
                <textarea
                  rows={3}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 md:p-8 max-w-sm w-full text-center space-y-5 animate-in fade-in zoom-in duration-200 relative">
            ...
            <p className="text-xs text-slate-600 italic">
              {calculatedScore >= 80
                ? 'Māsyā Allāh! Nilai kamu sangat luar biasa! Pertahankan akhlak muliamu ya!'
                : 'Bagus! Tetap semangat meneladani sifat mulia Nabi Adam a.s.!'}
            </p>
