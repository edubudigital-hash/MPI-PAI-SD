...
import { PROYEK_KELAS1_P4 } from '../../data/kelas1Pembelajaran4Data';
...
interface ProyekMuridView1_P4Props {
  grade?: number;
  lesson?: number;
}

export const ProyekMuridView1_P4: React.FC<ProyekMuridView1_P4Props> = ({
  grade = 1,
  lesson = 4,
}) => {
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <p>PAI & Budi Pekerti SD Kelas ${grade} • Pembelajaran ${lesson}: Mengenal dan Menerapkan Lima Rukun Islam serta Memahami Makna Syahadatain</p>
          </div>

          ${PROYEK_KELAS1_P4.map(
            (p, idx) => `
            <div class="card">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-bold px-2.5 py-0.5 bg-amber-100 text-amber-800 rounded-full">
              PjBL & PBL Kelas 1 Pembelajaran 4
            </span>
            <h3 className="text-xl font-black text-slate-800 mt-1">
              Proyek Murid & Pemecahan Masalah
            </h3>
            <p className="text-xs text-slate-500">
              4 Penugasan Berbasis Proyek (2 PjBL) dan Masalah (2 PBL) Rukun Islam & Syahadatain
            </p>
          </div>
        </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-600 font-medium space-y-1">
                <div className="font-extrabold text-emerald-700 flex items-center gap-1.5">
                  <Target className="w-4 h-4" /> Tujuan Penugasan:
                </div>
                <p>{item.objective}</p>
              </div>
