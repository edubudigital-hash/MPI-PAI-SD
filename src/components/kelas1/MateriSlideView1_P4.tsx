...
import { SLIDES_KELAS1_P4 } from '../../data/kelas1Pembelajaran4Data';
...
interface MateriSlideView1_P4Props {
  grade?: number;
  lesson?: number;
}

export const MateriSlideView1_P4: React.FC<MateriSlideView1_P4Props> = ({
  grade = 1,
  lesson = 4,
}) => {
  ...
  const currentSlide = SLIDES_KELAS1_P4[currentSlideIndex];

  const handleNext = () => {
    soundFX.playClick();
    if (currentSlideIndex < SLIDES_KELAS1_P4.length - 1) {
  ...
  const filteredSlides = SLIDES_KELAS1_P4.filter(
    (slide) =>
  ...
  const handleDownloadPDF = () => {
    ...
    const content = `
      ...
      <html>
        <head>
          <title>Materi PAI SD Kelas ${grade} - Pembelajaran ${lesson}: Rukun Islam & Syahadatain</title>
          <style>
            @page {
              size: A4;
              margin: 15mm;
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800">
              Materi Slide: 5 Rukun Islam & Syahadatain
            </h3>
            <p className="text-xs text-slate-500">
              Slide {currentSlideIndex + 1} dari {SLIDES_KELAS1_P4.length}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-emerald-100 shadow-xl overflow-hidden transition-all duration-300">
          ...
          <div className="p-6 md:p-8 bg-gradient-to-r from-emerald-700 via-teal-700 to-emerald-800 text-white flex items-center justify-between border-b border-emerald-600/50">
            <div className="space-y-1">
              <span className="text-xs font-black uppercase px-3 py-1 bg-white/20 text-emerald-100 rounded-full border border-white/30 backdrop-blur-xs">
                Slide {currentSlide.id} / {SLIDES_KELAS1_P4.length} • {currentSlide.badge}
              </span>
          ...
          <div className="p-5 md:p-6 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            ...
            <div className="text-xs font-extrabold text-slate-500 hidden sm:block">
              {currentSlideIndex + 1} / {SLIDES_KELAS1_P4.length}
            </div>

            <button
              onClick={handleNext}
              disabled={currentSlideIndex === SLIDES_KELAS1_P4.length - 1}
