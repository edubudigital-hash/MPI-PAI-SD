...
import { 
  EVALUASI_PG_KELAS6_P5, 
  EVALUASI_PGK_KELAS6_P5, 
  EVALUASI_MENJODOHKAN_KELAS6_P5, 
  EVALUASI_BENAR_SALAH_KELAS6_P5, 
  EVALUASI_URAIAN_KELAS6_P5 
} from '../../data/kelas6Pembelajaran5Data';
...
interface EvaluasiView6_P5Props {
  grade: number;
  lesson: number;
}

export const EvaluasiView6_P5: React.FC<EvaluasiView6_P5Props> = ({ grade, lesson }) => {
  ...
  const calculateResult = () => {
    let pgCorrect = 0;
    EVALUASI_PG_KELAS6_P5.forEach(q => {
      if (pgAnswers[q.id] === q.answer) pgCorrect++;
    });

    let pgkCorrect = 0;
    EVALUASI_PGK_KELAS6_P5.forEach(q => {
    ...
    EVALUASI_MENJODOHKAN_KELAS6_P5.forEach(q => {
      if (menjodohkanAnswers[q.id] === q.matchTarget) menjodohkanCorrect++;
    });

    let bsCorrect = 0;
    EVALUASI_BENAR_SALAH_KELAS6_P5.forEach(q => {
  ...
  const handlePrint = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Evaluasi Pembelajaran PAI - Kelas ${grade} Bab ${lesson}: Khulafaur Rasyidin</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <p>BAB ${lesson}: Kisah dan Jasa Khalifah Abu Bakar Ash-Shiddiq & Umar bin Al-Khattab (HOTS & Computational Thinking)</p>
          </div>

          <!-- BAGIAN A -->
          <div class="section-title">Bagian A: 10 Soal Pilihan Ganda (PG)</div>
          ${EVALUASI_PG_KELAS6_P5.map((q, idx) => `
            <div class="q-card">
          ...
          ${EVALUASI_PGK_KELAS6_P5.map((q, idx) => `
            <div class="q-card">
              <div class="q-title">${idx + 1}. ${q.question}</div>
              <div class="options-grid">
                ${q.options.map((opt, oIdx) => `
          ...
          <table class="table-match">
            ...
            <tbody>
              ${EVALUASI_MENJODOHKAN_KELAS6_P5.map((q, idx) => `
                <tr>
          ...
          ${EVALUASI_BENAR_SALAH_KELAS6_P5.map((q, idx) => `
            <div class="q-card">
              ...
              <div class="reason-box">
                <strong>Alasan / Pembahasan Sejarah:</strong> ${q.explanation}
              </div>
            </div>
          `).join('')}
          ...
          ${EVALUASI_URAIAN_KELAS6_P5.map((q, idx) => `
            <div class="q-card">
              <div class="q-title">${idx + 1}. ${q.question}</div>
          ...
          <div class="footer">
            Dokumen Evaluasi Pembelajaran PAI SD Kelas 6 - Bab 5 Khulafaur Rasyidin • Dicetak Otomatis
          </div>
          <script>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-extrabold text-white">
              Evaluasi Pembelajaran PAI Bab 5
            </h1>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 space-y-6 shadow-md">
        <div className="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Pilihlah Benar/Salah dan tuliskan alasan penjelasan sejarahnya pada kolom yang disediakan!</p>
          </div>
        </div>
              ...
              <div key={q.id} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 space-y-3">
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                    Tuliskan Alasan / Penjelasan Sejarahmu:
                  </label>
                  <input
                    type="text"
                    value={userReason}
                    onChange={(e) => handleReasonChangeBS(q.id, e.target.value)}
                    placeholder="Contoh: Karena Abu Bakar r.a. lahir di Makkah sekitar dua tahun setelah Tahun Gajah..."
