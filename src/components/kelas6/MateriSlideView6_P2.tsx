...
import { SLIDES_KELAS6_P2, SlideData } from '../../data/kelas6Pembelajaran2Data';
...
interface MateriSlideView6_P2Props {
  grade: number;
  lesson: number;
}
...
export const MateriSlideView6_P2: React.FC<MateriSlideView6_P2Props> = ({ grade, lesson }) => {
  const [currentSlideIndex, setCurrentSlideIndex] = useState(0);
  const [isPlayingAudio, setIsPlayingAudio] = useState(false);

  const currentSlide: SlideData = SLIDES_KELAS6_P2[currentSlideIndex];
  const totalSlides = SLIDES_KELAS6_P2.length;
  ...
  const handlePrint = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Materi Pembelajaran - Kelas ${grade} Bab ${lesson}: Mengenal dan Meneladani Asmaul Husna</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <p>BAB ${lesson}: Mengenal dan Meneladani Asmaul Husna: Al-Waliy, Al-Hamid, Al-'Adl, dan As-Sabur</p>
          </div>

          ${SLIDES_KELAS6_P2.map((slide, idx) => `
            <div class="slide-card">
              <div style="float: right; font-size: 11px; font-weight: bold; color: #0f766e; background-color: #ccfbf1; padding: 2px 8px; border-radius: 4px;">
                SLIDE ${idx + 1} OF ${SLIDES_KELAS6_P2.length}
              </div>
              ...
              ${slide.explanation ? `
                <div class="explanation">
                  <strong>Penjelasan Detail:</strong> ${slide.explanation}
                </div>
              ` : ''}
            </div>
          `).join('')}
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          {currentSlide.arabicText && (
            <div className="p-6 rounded-2xl bg-teal-50/80 dark:bg-teal-950/40 border-2 border-teal-200 dark:border-teal-800/80 text-center space-y-3">
              <div className="text-2xl sm:text-4xl font-bold text-teal-900 dark:text-teal-200 font-serif leading-loose">
                {currentSlide.arabicText}
              </div>
