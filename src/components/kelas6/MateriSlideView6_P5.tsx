...
import { SLIDES_KELAS6_P5, SlideData } from '../../data/kelas6Pembelajaran5Data';
...
interface MateriSlideView6_P5Props {
  grade: number;
  lesson: number;
}

export const MateriSlideView6_P5: React.FC<MateriSlideView6_P5Props> = ({ grade, lesson }) => {
  ...
  const currentSlide: SlideData = SLIDES_KELAS6_P5[currentSlideIndex];

  // Filtering slides for bookmark / search
  const filteredSlides = SLIDES_KELAS6_P5.filter(slide => {
  ...
  const handleNext = () => {
    if (currentSlideIndex < SLIDES_KELAS6_P5.length - 1) {
      soundFX.playClick();
  ...
  const handlePrintMateri = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Materi PAI SD Kelas ${grade} Bab ${lesson}: Khulafaur Rasyidin</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <p>BAB ${lesson}: Kisah dan Jasa Khalifah Abu Bakar Ash-Shiddiq serta Khalifah Umar bin Al-Khattab</p>
          </div>

          ${SLIDES_KELAS6_P5.map((s, idx) => `
            <div class="slide-card">
              ...
              ${s.tableData ? `
                <table class="table-custom">
                  <thead>
                    <tr>
                      <th>Poin Karakter Abu Bakar r.a.</th>
                      <th>Poin Karakter Umar r.a.</th>
                      ${s.tableData[0].col3 ? '<th>Aplikasi Murid</th>' : ''}
                    </tr>
                  </thead>
                  <tbody>
                    ${s.tableData.map(row => `
          ...
          <div class="footer">
            Dokumen Rangkuman Materi Pembelajaran PAI SD Kelas 6 - Bab 5 • Dicetak Otomatis
          </div>
          <script>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-extrabold text-white">
              Kisah dan Jasa Khalifah Abu Bakar Ash-Shiddiq & Umar bin Al-Khattab
            </h1>
            <p className="text-xs sm:text-sm text-teal-100 mt-1">
              Jelajahi 15 slide interaktif kisah Khulafaur Rasyidin lengkap dengan kuis pemahaman dan rangkuman.
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          >
            Semua Slide ({SLIDES_KELAS6_P5.length})
          </button>
          <button
            onClick={() => {
              soundFX.playClick();
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs text-slate-800 dark:text-slate-200 focus:outline-hidden focus:ring-2 focus:ring-teal-500"
          />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden min-h-[480px] flex flex-col justify-between">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/80 p-5 sm:p-6 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="w-9 h-9 rounded-2xl bg-teal-600 text-white font-extrabold text-sm flex items-center justify-center shrink-0 shadow-md">
              {currentSlide.id}
            </span>
            <div>
              <span className="text-[10px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider bg-teal-50 dark:bg-teal-950/60 px-2.5 py-0.5 rounded-full border border-teal-200 dark:border-teal-800">
                {currentSlide.badge}
              </span>
        ...
        <div className="p-6 sm:p-8 space-y-6 grow">
          ...
          {currentSlide.arabicText && (
            <div className="bg-emerald-50/60 dark:bg-emerald-950/30 border-r-4 border-emerald-600 p-4 sm:p-5 rounded-2xl space-y-2">
              <p className="text-right text-lg sm:text-xl font-bold font-serif text-slate-800 dark:text-slate-100 leading-loose">
                {currentSlide.arabicText}
              </p>
          ...
          {currentSlide.keyTerms && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {currentSlide.keyTerms.map((kt, idx) => (
                <div key={idx} className="p-3.5 rounded-2xl bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-800 space-y-1">
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-700">
              <table className="w-full text-xs text-left">
                <thead className="bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-bold border-b border-slate-200 dark:border-slate-700">
                  <tr>
                    <th className="p-3">Abu Bakar Ash-Shiddiq r.a.</th>
                    <th className="p-3">Umar bin Al-Khattab r.a.</th>
                    {currentSlide.tableData[0].col3 && <th className="p-3">Aplikasi Murid</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {currentSlide.tableData.map((row, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      <td className="p-3 font-medium text-slate-700 dark:text-slate-300">{row.col1}</td>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/80 p-4 sm:p-5 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between gap-4">
          ...
          <button
            onClick={handleNext}
            disabled={currentSlideIndex === SLIDES_KELAS6_P5.length - 1}
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all ${
              currentSlideIndex === SLIDES_KELAS6_P5.length - 1
                ? 'opacity-40 cursor-not-allowed bg-slate-200 dark:bg-slate-700 text-slate-400'
